...
//    shadowMap.CreateTextureAttachment(GL_DEPTH_ATTACHMENT, 1024, 1024);
        ...
        std::unordered_map<LightSource *, CubeTexture *> map;

        auto depthCubeMap = new CubeTexture(SHADOW_WIDTH, SHADOW_HEIGHT, GL_DEPTH_COMPONENT, GL_DEPTH_COMPONENT,
                                            GL_FLOAT);
        depthCubeMap->Bind();
        ...
        uint64_t handle = glGetTextureHandleARB(depthCubeMap->id);
        glMakeTextureHandleResidentARB(handle);
        map[scene.Lights()[0]] = depthCubeMap;
        scene.Lights()[0]->shadow = handle;
        ...
        while (true) {
            ...
            auto cube = map[light];
            shadowMap.SetTextureAttachment(GL_DEPTH_ATTACHMENT, cube);
